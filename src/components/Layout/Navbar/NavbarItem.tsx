...
const NavbarItem: FC<NavbarItemProps> = ({ text, type, tag }) => {
    ...
    return (
        <Box
            position={type === 'dropdown' ? 'unset' : 'relative'}
        ...
        >
            <Box position={'relative'}>
                <Box
                    ...
                    color={`${tag === 'sale' && mainColor.saleTag}`}
                >
                    {text}
                </Box>
            ...
            <Box
                ...
                left={renderEle()?.left}
                top="100%"
                width={renderEle()?.width}
                sx={{ zIndex: 99 }}
            >
                <Box
                    paddingTop={type === 'dropdown' ? 4 : 0}
                    bg={mainColor.white}
                >
                    <SubMenu
                        icon={
                            <Image
                                src={ImageAssets.Electronics}
                                alt="Electronis"
                                className="image"
                            />
                        }
